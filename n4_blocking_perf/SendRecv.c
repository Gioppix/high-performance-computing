    ...
    for (n = 1; n < 1100000; n *= 2) {
        ...
        for (k = 0; k < NUMBER_OF_TESTS; k++) {
            if (rank == 0) {
                ...
                MPI_Sendrecv(MPI_BOTTOM, 0, MPI_INT, 1, 14, MPI_BOTTOM, 0, MPI_INT, 1, 14,
                ...
                /* Make sure both processes are ready */
                MPI_Sendrecv(MPI_BOTTOM, 0, MPI_INT, 0, 14, MPI_BOTTOM, 0, MPI_INT, 0, 14,
                             MPI_COMM_WORLD, &status);
                for (j = 0; j < nloop; j++) {
        ...
        if (rank == 0) {
            double rate;
            if (tmin > 0)
                rate = n * sizeof(double) * 1.0e-6 / tmin;
            else
                rate = 0.0;
